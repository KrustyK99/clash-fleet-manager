...
function deriveBuilderCapacityFromSnapshot(snapshot) {
  ...
  const hasCompletedBobControl = Number.isFinite(bobControlLevel) && bobControlLevel >= 5;
  const hasSixthHomeBuilder = homeBuilderHuts >= 5 && (hasHomeBobMarker || hasCompletedBobControl);
  let homeTotal = null;

  if (homeBuilderHuts > 0) {
    // The sixth home builder should only be counted after the account already has
    // five normal Builder's Huts. Some lower-level accounts can include B.O.B-like
    // marker IDs in the Home Village export before they actually have six home
    // builders; without this guard those accounts are incorrectly promoted to 6.
    homeTotal = hasSixthHomeBuilder ? Math.max(6, homeBuilderHuts) : homeBuilderHuts;
  }
